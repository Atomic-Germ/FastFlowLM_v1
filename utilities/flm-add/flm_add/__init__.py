...
# Q4NX (closed stack) model files
REQUIRED_FILES_Q4NX = ["config.json", "model.q4nx", "tokenizer.json", "tokenizer_config.json"]
OPTIONAL_FILES_Q4NX = ["chat_template.jinja", "vision_weight.q4nx", "audio_weight.q4nx"]
ALL_FILES_Q4NX = REQUIRED_FILES_Q4NX + OPTIONAL_FILES_Q4NX

# Embedding (open stack) model files
REQUIRED_FILES_EMBED = ["config.json", "model.safetensors", "tokenizer.json", "tokenizer_config.json"]
OPTIONAL_FILES_EMBED = ["chat_template.jinja", "special_tokens_map.json", "tokenizer.model"]
ALL_FILES_EMBED = REQUIRED_FILES_EMBED + OPTIONAL_FILES_EMBED

# Legacy constants for backward compatibility
REQUIRED_FILES = REQUIRED_FILES_Q4NX
OPTIONAL_FILES = OPTIONAL_FILES_Q4NX
ALL_FILES = ALL_FILES_Q4NX

# ---------------------------------------------------------------- frontmatter / base model

def parse_frontmatter(text):
    """Parse YAML frontmatter from README.md content. Returns (meta_dict, body)."""
    ...
    if len(parts) < 3:
        return {}, text
    if yaml is None:
        raise SystemExit("PyYAML required to parse README frontmatter. `pip install pyyaml`")
    meta = yaml.safe_load(parts[1]) or {}
    body = parts[2].lstrip()
    return meta, body


def hf_readme(repo_id):
    """Fetch README.md content from Hugging Face."""
...
def is_embedding_model(meta, tags=None):
    """Check if model metadata indicates an embedding model."""
    if tags is None:
        tags = meta.get("tags") or []
    pipeline = meta.get("pipeline_tag") or ""
    return (
        pipeline == "sentence-similarity"
        or "sentence-similarity" in tags
        or "text-embeddings-inference" in tags
        or meta.get("library_name") == "sentence-transformers"
    )


def extract_base_model(meta):
    """Extract base_model from frontmatter. Returns repo_id or None."""
...
def is_embedding_repo(repo_id):
    """Quick check if a repo is an embedding model by reading its README frontmatter."""
    readme = hf_readme(repo_id)
    if not readme:
        return False
    meta, _ = parse_frontmatter(readme)
    return is_embedding_model(meta)
...
FAMILY_ALIASES = [
    # Sorted longest-prefix-first so "qwen3.5" matches before "qwen3", etc.
...
def _extract_size(bare):
    ...
    # Handle B/b (billion) and M/m (million) suffixes.
    m = re.search(r'(\d+(?:\.\d+)?[BbMm](?:-[A-Za-z]+\d+(?:\.\d+)?[A-Za-z]*)*)', bare)
    if not m:
        return None, bare
...
def derive_family(system_registry, dir_name, explicit=None, base_entry=None):
    ...
    # Generic: every Qwen 3.x dense model (3.5, 3.7, 3.8, …) uses the qwen3.5 engine.
    # Excludes 3.6-moe which has its own engine.
    if re.match(r"qwen3\.\d", lower) and "moe" not in lower:
        return "qwen3.5"
...
def fetch_assets(repo_id, target, modelscope=False, verify=True, force=False, quiet=False, embedding=False, base_model=None):
    ...
    for e in hf_file_tree(repo_id):
        ...
        if p and "/" not in p:
            entries[p] = e
    req, opt, all_files = _file_lists(embedding)
    for fname in all_files:
        if fname not in entries:
            continue
        dest = target / fname
...
def _file_lists(embedding):
    if embedding:
        return REQUIRED_FILES_EMBED, OPTIONAL_FILES_EMBED, ALL_FILES_EMBED
    return REQUIRED_FILES_Q4NX, OPTIONAL_FILES_Q4NX, ALL_FILES_Q4NX


def copy_from_dir(src_dir, target, force=False, embedding=False):
    obtained = []
    req, opt, all_files = _file_lists(embedding)
    for fname in all_files:
        src = src_dir / fname
...
def _safetensors_offsets(safetensors_path, tensor_names):
    """Read .safetensors header to get tensor offsets and shapes."""
    import struct
...
def build_manifest(target, base_model_repo, config, manifest_name="weights_manifest.json"):
    """Generate weights_manifest.json with relative paths and offsets from .safetensors."""
    import struct
    ...
    offsets = _safetensors_offsets(target / "model.safetensors", tensor_names) if (target / "model.safetensors").is_file() else {}
    ...
    manifest_path = target / "weights_manifest.json"
    save_json(manifest_path, manifest)
    log(f"[INFO] Generated weights_manifest.json with {len(offsets)} tensors")
...
def _safetensors_offsets(safetensors_path, tensor_names):
    ...
    offsets = {}
    for name in tensor_names:
        if name in header and name != "__metadata__":
            info = header[name]
...
def build_embed_entry(base_entry, dir_name, files, size, embedding_family):
    entry = dict(base_entry) if base_entry else {}
    entry["name"] = dir_name
    entry["files"] = list(files)
    entry["url"] = ""
    ...
    entry["ms_url"] = ""
    entry.setdefault("max_prefill_len", 4096)
    entry.setdefault("default_context_length", 2048)
    entry.setdefault("flm_min_version", "0.9.45")
    entry.setdefault("details", {}).setdefault("format", "NPU2")
    entry["details"]["family"] = "embed-gemma"
    return entry


def _embedding_tag(dir_name):
    """Derive embedding tag like 'embed-gemma:300m' from directory name."""
    size, rest = _extract_size(_strip_npu2(dir_name))
    if not size:
        raise SystemExit(f"Could not derive size from '{dir_name}'")
    tokens = [t for t in re.split(r"[-_ ]+", rest) if t]
    if not tokens:
        raise SystemExit(f"Could not derive tag from '{dir_name}'")
    # For Embedding-Gemma, the family is 'gemma' (second token), not 'embedding' (first)
    family = tokens[1].lower() if len(tokens) > 1 and tokens[0].lower() == 'embedding' else tokens[0].lower()
    return f"embed-{family}:{size}"
def main():
    ap = argparse.ArgumentParser(
        description="Install a pre-converted FLM (Q4NX) model or embedding model and register it with FastFlowLM.",
    )
    ap.add_argument("repo", help="Hugging Face repo id (Org/Name), ModelScope id (with --modelscope), URL, or local directory")
    ap.add_argument("--tag", help="Registry tag (default: derived from the repo name, e.g. qwen3.5-claude:9b or embed-gemma:300m)")
    ...
    # --- Detect embedding model via README frontmatter ---
    embedding = False
    base_model_repo = None
    if not local_dir and not modelscope:
        readme_text = hf_readme(repo)
        if readme_text:
            meta, _ = parse_frontmatter(readme_text)
            embedding = is_embedding_model(meta)
            base_model_repo = extract_base_model(meta)
            if embedding and not args.quiet:
                log(f"[INFO] Detected embedding model (base: {base_model_repo})")
    ...
    # Tag derivation: embedding models use embed-<family>:<size> format
    if embedding:
        tag = _embedding_tag(dir_name)
        family = "embed-gemma"
    else:
    ...
    # For embedding models, xclbins come from the embedding NPU artifacts (npu_matmul_f32)
    if embedding:
        xclbin_source = None  # Will install npu_matmul_f32 artifacts instead
    ...
    if local_dir:
        ...
        target.mkdir(parents=True, exist_ok=True)
        files = copy_from_dir(local_dir, target, force=args.force, embedding=embedding)
    else:
        ...
        if snapshot:
            ...
            target.mkdir(parents=True, exist_ok=True)
            files = copy_from_dir(snapshot, target, force=args.force, embedding=embedding)
        else:
            if not args.quiet:
                log(f"[INFO] Downloading model files from {'ModelScope' if args.modelscope else 'Hugging Face'}: {repo}")
            target.mkdir(parents=True, exist_ok=True)
            files = fetch_assets(repo, target, args.modelscope, verify=not args.no_verify, force=args.force, quiet=args.quiet, embedding=embedding)

    # For embedding models, also fetch base_model weights/tokenizer if not present
    if embedding and base_model_repo:
        log(f"[INFO] Fetching base model weights/tokenizer from {base_model_repo}...")
        base_files = fetch_assets(base_model_repo, target, False, verify=not args.no_verify, force=args.force, quiet=args.quiet, embedding=True)
        files.extend(base_files)
        # Re-deduplicate
        files = list(dict.fromkeys(files))

    # For embedding models, generate weights_manifest.json
    if embedding:
        if not args.quiet:
            log("[INFO] Generating weights_manifest.json...")
        build_manifest(target, base_model_repo, target / "config.json")

    missing = [f for f in (REQUIRED_FILES_EMBED if embedding else REQUIRED_FILES) if not (target / f).is_file()]
    if missing:
    ...
    if embedding:
        entry = build_embed_entry(base_entry, dir_name, files, size_value, "embed-gemma")
        # Embedding tag format
        tag = _embedding_tag(dir_name)
    else:
    ...
    if embedding:
        # Install NPU matmul artifacts for embedding engine
        if not args.no_xclbin:
            log("[INFO] Installing NPU matmul artifacts for embedding engine...")
            # Check multiple locations for the NPU artifacts (bf16 output in npu_matmul/, f32 in npu_matmul_f32/)
            candidates = [
                Path("/home/atomic-germ/.config/flm/models/Embedding-Gemma-300M-NPU2/npu_matmul"),
                Path("/home/atomic-germ/.config/flm/models/Embedding-Gemma-300M-NPU2/npu_matmul_f32"),
                target / "npu_matmul_f32",
            ]
            npu_src = None
            for c in candidates:
                if c.is_dir():
                    npu_src = c
                    break
            if npu_src and npu_src.is_dir():
                npu_dst = target / "npu_matmul_f32"
                if npu_dst.exists():
                    shutil.rmtree(npu_dst)
                shutil.copytree(npu_src, npu_dst)
                log(f"[INFO] Installed NPU matmul artifacts to {npu_dst}")
            else:
                log("[WARN] NPU matmul artifacts not found. Install manually.")
    else:
        if not args.no_xclbin:
